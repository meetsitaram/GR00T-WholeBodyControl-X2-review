...
def _parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--list", action="store_true", dest="list_motions",
        help="With --pkl: list the motion keys inside that PKL. Without "
             "--pkl: list the walk/locomotion PKLs shipped under "
             "gear_sonic/data/motions/ (x2_walk_examples/, "
             "x2_g1_recorded_walks/). No stack needed; exits after listing.",
    )
    parser.add_argument(
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.list_motions:
        if args.pkl is not None:
            ...
            print(f"{args.pkl}: {len(data)} clip(s)")
            for k, m in data.items():
                n = m["dof"].shape[0]
                fps = float(m.get("fps", 30.0))
                print(f"  {k}  ({n} frames @ {fps:g} fps = {n / fps:.1f}s)")
        else:
            root = Path(__file__).resolve().parents[2] / "gear_sonic/data/motions"
            for sub in ("x2_walk_examples", "x2_g1_recorded_walks"):
                d = root / sub
                pkls = sorted(d.glob("*.pkl")) if d.is_dir() else []
                print(f"{d.relative_to(root.parents[2])}: {len(pkls)} pkl(s)")
                for p in pkls:
                    print(f"  {p.name}")
            print(
                "\nplay one:  play_locomotion.py --pkl "
                "gear_sonic/data/motions/x2_walk_examples/relaxed_walk_forward.pkl"
            )
        return 0
    ...
    # Bare-name convenience: `--pkl relaxed_walk_forward[.pkl]` resolves
    # against the shipped walk dirs when the path doesn't exist as given.
    if not args.pkl.exists() and args.pkl.parent == Path("."):
        root = Path(__file__).resolve().parents[2] / "gear_sonic/data/motions"
        name = args.pkl.name if args.pkl.suffix == ".pkl" else args.pkl.name + ".pkl"
        for sub in ("x2_walk_examples", "x2_g1_recorded_walks", "."):
            cand = root / sub / name
            if cand.is_file():
                print(f"[play_locomotion] resolved {args.pkl} -> {cand}", flush=True)
                args.pkl = cand
                break
        else:
            raise SystemExit(
                f"play_locomotion: {args.pkl} not found (cwd or shipped walk "
                "dirs). Run with --list to see available pkls."
            )
