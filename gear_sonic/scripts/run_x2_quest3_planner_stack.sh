...
#   --model CLI arg > X2_PLANNER_SMOKE_MODEL env > model cache
#   ($SONIC_X2_MODELS > $SONIC_HOME/x2) > legacy machine-local checkpoint.
if [[ -n "${X2_PLANNER_SMOKE_MODEL:-}" ]]; then
...
build_pad_bridge_extra() {
    ...
    # Defaults mirror the REAL ROBOT's ritual bank layout
    # (x2_pc2/ritual_start_demo.sh): one bank per face button —
    #   L1+Y easy dances · L1+X combat · L1+B medium dances ·
    #   L1+A gestures · right-stick 4-way turns · L1+R1 stop.
    # walk_circle_001 (robot D-pad UP) is deliberately NOT wired.
    # Export PAD_CLIP_PKL="" to disarm the chords entirely.
    if [[ -z "${PAD_CLIP_PKL+x}" ]]; then
        PAD_CLIP_PKL="${REPO_ROOT}/gear_sonic/data/motions/x2_pad_banks.pkl"
        : "${PAD_CLIP_KEYS:=dance_party_hips_003__A467,dance_party_hips_003__A464,dance_party_hips_003__A465}"
        : "${PAD_CLIP_KEYS_X:=shadow_boxing_R_003__A359_M,shadow_boxing_R_003__A359}"
        : "${PAD_CLIP_KEYS_M:=egipt_dance_R_001__A438,dance_hiphop_stick_n_roll_dancehall_R_loop_003__A324,dance_distraction_dance_001__A466}"
        : "${PAD_CLIP_KEYS_G:=right_wave_001,right_kiss_001,right_five_001,right_shake_001,turn_wave_right_001,turn_wave_left_001}"
        # LEFT,RIGHT,UP,DOWN (exactly 4). Robot maps DOWN to walk_circle_001
        # (removed as unsafe); we reuse the straight walk there instead.
        : "${PAD_CLIP_KEYS_TURN:=locowalk__idle_turn_270_002__A056_M,locowalk__idle_turn_270_002__A056,relaxed_walk_forward,relaxed_walk_forward}"
    fi
...
if [[ "${VLA_MODE}" -eq 1 ]]; then
    ...
    if [[ "${VLA_NO_POLICY}" -eq 1 ]]; then
        ...
        # gear_sonic/scripts/vla/inspect_vla_chunks.py for a quick summary). Skipped in
