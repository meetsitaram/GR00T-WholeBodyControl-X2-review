#!/bin/bash
# Elastic multi-node SONIC training for Asimov on the Nebius GPU cluster.
#
# Run the SAME script on EVERY node (reserved-main + preemptible workers):
#
#   tmux new -d -s elastic "bash gear_sonic/scripts/cloud/run_asimov_elastic.sh"
#
...
#   EXP_NAME         Hydra +exp= leaf                  (default sonic_asimov_loco)
...
RDZV_ENDPOINT=${RDZV_ENDPOINT:-10.0.0.37:29400}
NNODES_MIN=${NNODES_MIN:-3}
NNODES_MAX=${NNODES_MAX:-4}
NPROC_PER_NODE=${NPROC_PER_NODE:-8}
NUM_ENVS=${NUM_ENVS:-8192}
NUM_ITERS=${NUM_ITERS:-200000}
EXP_NAME=${EXP_NAME:-sonic_asimov_loco}
MOTION_FILE=${MOTION_FILE:-/mnt/shared/corpus/asimov_sonic_executed_feasible.pkl}
