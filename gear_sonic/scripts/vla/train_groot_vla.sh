...
if [[ ! -f "$REPO_ROOT/gear_sonic/scripts/vla/launch_finetune_x2.py" ]]; then
    echo -e "${RED}[FATAL] gear_sonic/scripts/vla/launch_finetune_x2.py not found.${NC}" >&2
    exit 2
fi
...
echo "[finetune] cmd:    python gear_sonic/scripts/vla/launch_finetune_x2.py '"${TRAIN_ARGS[*]}"'"

PYTHONPATH=external_dependencies/Isaac-GR00T:. python \
    gear_sonic/scripts/vla/launch_finetune_x2.py \
    '"${TRAIN_ARGS[*]}"'
