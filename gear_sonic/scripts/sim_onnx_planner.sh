...
# alone is overridable via $SONIC_X2_MODELS. Cache artifacts join the
# md5 candidate pools below -- identity gating vs the robot is unchanged:
# explicit SONIC_MODEL/PLANNER_MODEL env > md5 auto-match (cloud
# checkpoints AND the cache) > legacy dirs.
