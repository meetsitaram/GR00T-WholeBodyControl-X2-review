...
# gear_sonic/utils/teleop/x2_hand_retarget.py for the rationale.
_INTENTIONAL_ANCHOR_SHIFT_MOTOR_INDICES: tuple[int, ...] = (
    0,  # thumb_roll
    1,  # thumb_abad
)
...
def test_hand_q_match_within_float32_ulp(replay_results, side):
    diff_full = replay_results[f"hand_diff_{side}"][replay_results["mask"]]
    # Drop the columns that have a documented OPEN-anchor shift vs v6.
    keep_cols = [
        i for i in range(diff_full.shape[1])
        if i not in _INTENTIONAL_ANCHOR_SHIFT_MOTOR_INDICES
    ]
    diff = diff_full[:, keep_cols]
    assert diff.max() < HAND_AND_FILTER_LINF, (
        f"{side}_hand_q diverged from v6 on UNCHANGED motors: "
        f"Linf={diff.max():.3e} > {HAND_AND_FILTER_LINF:.0e}. The XRHand "
        "fast path / per-finger map / finger filter has a real (not "
        "float32 round-off) regression on a motor whose anchors didn't "
        "change. (thumb_roll / thumb_abad were intentionally re-anchored "
        "on 2026-05-13 -- those columns are excluded above.)"
    )
