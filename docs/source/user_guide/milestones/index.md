...
| 2026-06-08 | Full-arm SONIC bypass (`--wrist-bypass=ik-arms`) | **Real X2 + sim**, Quest 3 stack | After wrist-offset v1 cut calibration-time orientation error to ~10°, SONIC's whole-arm policy output was still fighting the operator on slow workspace-corner targets. Extended the existing surgical `--wrist-bypass=ik` (4 wrist DOFs) to a new `--wrist-bypass=ik-arms` mode covering the **full 14-DOF arm** (MJ 15..28 = both shoulders + elbows + wrist_yaw + wrist_pitch + wrist_roll). SONIC's 31-DOF policy still runs in full and drives legs (0..11), waist (12..14), and head (29..30) for balance; only the arm slots of the final PD target get overwritten with the IK reference, BEFORE the safety stack (so soft-start + per-family clamps + tilt-trip still apply). Tokenizer obs is unchanged (policy sees IK ref for all 31 DOFs), same compromise as wrist-bypass. Same C++ binary covers sim + real (`deploy_x2.sh sim` and PC2 native colcon). 2 new unit tests pin the 14-slot index table + a wrist-subset invariant. Python lock-step port at `gear_sonic/utils/teleop/wrist_bypass.py`. **Stability**: arms have larger inertial effect than wrists; do sim rollout first, real-robot rollout with a spotter / fixture. Rollback is one daemon restart away (`--wrist-bypass ik`). |
