...
gpu-cluster-node-reserved-main   RESERVED (never preempted)
  ├─ c10d rendezvous store  10.0.0.37:29400   (RDZV_IS_HOST=1)
...
(`gear_sonic/scripts/cloud/run_asimov_elastic.sh`). Accelerate picks up
...
- **`--rdzv-conf is_host=1` is mandatory on the endpoint node.** torchrun's
  ...
  node times out as a client ("client socket has timed out ... 10.0.0.37,
...
- `gear_sonic/scripts/cloud/run_asimov_elastic.sh` — elastic supervisor (any robot/exp)
