...
start_one() {
  # $2 (optional) overrides the script path. Default is the laptop-
  # autogenerated tmux body; the DEPLOY must NOT use that (see below).
  S=$1; SCRIPT=${2:-/home/run/getsolo/log/start_${S}.sh}
...
# Deploy uses the PC2-RESIDENT ritual launcher (checked into the repo,
# staged by pc2_bringup.sh), NEVER log/start_x2_deploy.sh: that body is
# regenerated by every laptop x2_pc2_daemons.sh start with that session's
# flags, and one without --no-confirm leaves the deploy stuck at a y/N
# prompt no gamepad can answer (2026-07-29 blocked-ignition incident).
start_one x2_deploy /home/run/getsolo/start_x2_deploy_ritual.sh
