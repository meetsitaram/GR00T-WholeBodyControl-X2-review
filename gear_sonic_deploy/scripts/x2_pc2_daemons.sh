...
#
#     gear_sonic/scripts/vla/run_x2_vla_runtime.sh --enable-takeover ...
#
...
if [[ ${#_legacy_hits[@]} -gt 0 ]]; then
    ...
    printf '\e[31m[pc2 ERROR]\e[0m   gear_sonic/scripts/vla/run_x2_vla_runtime.sh --enable-takeover ...\n' >&2
...
tmux_kill_session() {
    ...
    ssh_pc2 "echo \"\$(date +%F_%T) ${name}: KILLED by x2_pc2_daemons.sh stop\" \
             >> /home/run/gear-sonic/log/ritual_fired.log 2>/dev/null || true"
...
cmd_start() {
    ...
    if [[ -n "${X2_TUNING}" ]]; then
        ...
        # on PC2 (i.e. /home/run/gear-sonic/gear_sonic_deploy/), so absolute
